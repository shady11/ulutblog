...
# look for liveFlag ********************************************************************************************  
     public function checkliveFlag($table, $where="") {
		$oldwhere = $where;
		$mysqlclassurl = strtolower($_SERVER['SCRIPT_NAME']);
		$mysmysqlclassurl_ar = explode("/",$mysqlclassurl);
		if ($table=='flats') $where = empty($where) ? "status='1'" : $where .= " AND status='1'";
		if ($table=='users') $where = empty($where) ? "liveFlag='1'" : $where .= " AND liveFlag='1'";
		//if ($table=='users') $where = empty($where) ? "user_status='1'" : $where = "AND user_status='1'";
		if ($mysmysqlclassurl_ar[1]!="cms"){
			if ($table=='flats' || $table=='rooms'){
				if(!empty($where)) $where .= " AND liveFlag=1";
				else $where = "liveFlag=1";
			} 
		}
		if (!($mysmysqlclassurl_ar[1]=="cms" || $mysmysqlclassurl_ar[1]=="control")){
			if ($table=='flats'){
				if(!empty($where)) $where .= " AND approved=1";
				else $where = "approved=1";
			} 
		}
		if ($mysmysqlclassurl_ar[1]=="admin" && $table=='flats'){
			$where = empty($oldwhere) ? "liveFlag='1'" : $oldwhere .= " AND liveFlag='1'";
		}
		return $where;
      }    
